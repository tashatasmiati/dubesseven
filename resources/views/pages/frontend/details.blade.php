    ...
    <section class="container mx-auto">
      <div class="flex flex-wrap my-4 md:my-12">
        <div class="w-full md:hidden px-4">
          <h2 class="text-5xl font-semibold">{{ $produk -> namaproduk }}</h2>
          <span class="text-xl">IDR {{ number_format ($produk->hargaproduk) }}</span>
        </div>
        <div class="flex-1">
          <div class="slider">
            <div class="thumbnail">
              @foreach ($produk->gallery as $item)
              <div class="px-2">
                <div
                  class="item {{$loop -> first ? 'selected' : ''}}"
                  data-img="{{ Storage::url($item->gambar)}}"
                >
                  <img
            ...
            <div class="preview">
              <div class="item rounded-lg h-full overflow-hidden">
                <img
                src="{{ $produk->gallery()->exists() ? Storage::url($produk->gallery->first()->gambar) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
                  alt="front"
                  class="object-cover w-full h-full rounded-lg"
                />
              </div>
        ...
        <div class="flex-1 px-4 md:p-6">
          <h2 class="text-5xl font-semibold">{{ $produk -> namaproduk }}</h2>
          <p class="text-xl">IDR {{ number_format ($produk->hargaproduk) }}</p>
            ...
            <div class="px-3 flex-none" style="width: 320px">
              <div class="rounded-xl p-4 pb-8 relative bg-white">
                <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                  <img
                    src="{{ $recommendation->gallery()->exists() ? Storage::url($recommendation->gallery->first()->gambar) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
                    alt=""
                    class="w-full h-full object-cover object-center"
                  />
                </div>
                <h5 class="text-lg font-semibold mt-4">{{ $recommendation->namaproduk }}</h5>
                <span class="">IDR {{ number_format($recommendation->hargaproduk) }}</span>
